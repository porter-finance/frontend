### How to start a new auction using the Porter Safe App

#### Connect Wallet & App

1. First, you will need to have a [Porter Safe](https://gnosis-safe.io/) that you control deployed in the network of interest. We suggest everyone to try it out on Rinkeby.

2. Once you have the Porter Safe created, you can see the following:

<img src="/assets/GA_SafeApp_1.png" alt="drawing" width="500"/>

3. In order to add the Porter Finance Safe App you need to click on the APPS section at the left-hand side of the UI, which will prompt you to the following page:

<img src="/assets/GA_SafeApp_2.png" alt="drawing" width="500"/>

#### Creating an Auction

4. Once you have opened the Porter Finance Safe App, you can see the list of parameters you must select to launch an auction.
...
(c) The third parameter you need to enter is the amount of tokens (expressed in numbers) you are about to auction. Keep in mind that the value of this parameter has to be within the amount of tokens that you hold in the Porter Safe.

As you can see in the image below, if you try to input a value of more tokens than are held in the Porter Safe, the app will show a “Not enough Balance” error.
...
8. That's it! You have successfully set up and deployed an auction using the Porter Safe App for Porter Finance.
...
PS: We strongly recommend auctioneers to use the method described above instead of using the script, since we believe that the Porter Safe is better suited for handling large amounts of digital assets
